...
import { VscSymbolMethod, VscAdd, VscPlay, VscServer } from 'react-icons/vsc';
import type { Tool, MCPTool } from '@/types';

type ActiveTab = 'agent' | 'mcp';

// ── MCP direct invocation ─────────────────────────────────────────

function MCPToolDetail({ tool, onBack }: { tool: MCPTool; onBack: () => void }) {
  ...
  return (
    <div className="p-3 space-y-3">
      <button type="button" onClick={onBack} className="text-xs text-accent-blue hover:underline">
        Back to MCP tools
      </button>
      <div>
        <div className="flex items-center gap-2">
          <div className="text-sm font-medium text-text-primary">{tool.name}</div>
          <span className="text-[9px] px-1.5 py-0.5 bg-accent-blue/10 text-accent-blue rounded">MCP</span>
        </div>
        <div className="text-xs text-text-secondary mt-0.5">{tool.description}</div>
        <div className="text-[10px] text-text-muted mt-0.5">{tool.category}</div>
      </div>
            ...
            <div key={key}>
              ...
              {schema.enum ? (
                <select
                  ...
                  aria-label={key}
                  className="w-full px-2 py-1 text-xs bg-bg-panel border border-border-default rounded text-text-primary focus:outline-none focus:ring-2 focus:ring-accent mt-0.5"
                >
      ...
      <button
        ...
        disabled={running}
        className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-accent-blue text-white rounded hover:opacity-90 transition-opacity disabled:opacity-50 w-full justify-center"
      >
...
function MCPToolList() {
  ...
  if (selected) return <MCPToolDetail tool={selected} onBack={() => setSelected(null)} />;

  const grouped = mcpTools.reduce<Record<string, MCPTool[]>>((acc, t) => {
    const cat = t.category ?? 'General';
    (acc[cat] = acc[cat] || []).push(t);
    return acc;
  }, {});

  if (loading) return <div className="p-3 text-xs text-text-muted">Loading MCP tools…</div>;
  if (mcpTools.length === 0) return <div className="p-3 text-xs text-text-muted text-center py-4">No MCP tools available</div>;

  return (
    <div className="p-3 space-y-3 overflow-y-auto flex-1">
      <div className="flex items-center gap-2 p-2 bg-accent-blue/5 border border-accent-blue/20 rounded text-[10px] text-accent-blue">
        <VscServer size={12} />
        <span>{mcpTools.length} tools · Built-in MCP Server · /mcp/tools</span>
      </div>
...
function AgentToolList() {
  ...
  const handleCreate = async () => {
    ...
    await createCustomTool({ name: newName.trim(), description: newDesc.trim(), prompt_template: newPrompt.trim() });
      ...
      <div className="p-3 space-y-3 overflow-y-auto flex-1">
        ...
        {showCreate && (
          <div className="p-3 bg-bg-secondary rounded-md space-y-2">
            <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Tool name"
            ...
            <input type="text" value={newDesc} onChange={(e) => setNewDesc(e.target.value)} placeholder="Description"
              className="w-full px-2 py-1.5 text-xs bg-bg-panel border border-border-default rounded text-text-primary placeholder:text-text-muted focus:outline-none focus:ring-2 focus:ring-accent" />
            <textarea value={newPrompt} onChange={(e) => setNewPrompt(e.target.value)} placeholder="Prompt template (use {{$input}} for variables)" rows={4}
            ...
            <div className="flex gap-2">
              <button type="button" onClick={handleCreate} className="flex-1 px-3 py-1.5 text-xs bg-accent text-bg-primary rounded hover:bg-accent-bright">Create</button>
              <button type="button" onClick={() => setShowCreate(false)} className="px-3 py-1.5 text-xs text-text-muted hover:text-text-primary">Cancel</button>
            </div>
          </div>
        )}
        {loading ? (
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex border-b border-border-default shrink-0">
        {(['agent', 'mcp'] as ActiveTab[]).map((t) => (
          ...
          >
            {t === 'agent' ? 'Agent Tools' : 'MCP Tools'}
          </button>
        ))}
      </div>
      <div className="flex-1 overflow-hidden">
        {tab === 'agent' ? <AgentToolList /> : <MCPToolList />}
      </div>
    </div>
  );
}
